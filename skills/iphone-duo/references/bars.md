...
- 내부 디스플레이가 세로 방향일 때는 HIG상 예외로 표준 가로 바를 유지할 만큼 세로 공간이 있다. 모든 자세에서 세로 바로 강제 전환하지 않는다.
- 멀티태스킹으로 내부 디스플레이를 좌우로 나눠 쓸 때 각 앱은 자신의 바깥쪽 가장자리에 컨트롤을 둔다(왼쪽 앱은 왼쪽, 오른쪽 앱은 오른쪽). 상대 앱의 컨트롤과 겹치지 않도록 safe area를 확인한다.
